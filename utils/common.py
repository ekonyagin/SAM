...
def vis_faces_with_age(hooks_dict, fig, gs, i):
    ...
    plt.title(
        "Input\nOut Sim={:.2f}\nInput Age={:.2f}".format(
            float(hooks_dict["diff_input_real"]), float(hooks_dict["input_age_real"])
        )
    )
    fig.add_subplot(gs[i, 1])
    plt.imshow(hooks_dict["target_face"])
    plt.title(
        "Target\nIn={:.2f},Out={:.2f}\nTarget Age={:.2f}".format(
            float(hooks_dict["diff_views_real"]),
    ...
    plt.title(
        "Output\nTarget Sim={:.2f}\nOuput Age={:.2f}".format(
            float(hooks_dict["diff_target_real"]), float(hooks_dict["output_age_real"])
        )
    )
    ...
    plt.title(
        "Recovered\nTarget Sim={:.2f}\nOuput Age={:.2f}".format(
            float(hooks_dict["diff_target_cycle"]),
