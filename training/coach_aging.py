...
class Coach:
    ...
    def __set_target_to_source(self, x, input_ages):
        return [
            torch.cat(
                (img, age * torch.ones((1, img.shape[1], img.shape[2])).to(self.device))
            )
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                input_ages = self.aging_loss.extract_ages(x) / 100.0

                # perform no aging in 33% of the time
                no_aging = random.random() <= (1.0 / 3)
                if no_aging:
                ...
                y_hat_inverse = self.__set_target_to_source(
                    x=y_hat_clone, input_ages=input_ages_clone
                )
                ...
                if self.global_step % self.opts.image_interval == 0 or (
                    ...
                    self.parse_and_log_images(
                        id_logs, x, y, y_hat, y_recovered, title="images/train/faces"
                    )
    ...
    def validate(self):
        ...
        for batch_idx, batch in enumerate(self.test_dataloader):
            ...
            with torch.no_grad():
                ...
                cur_loss_dict["loss"] = (
                    cur_loss_dict["loss_real"] + cur_loss_dict["loss_cycle"]
                )
    ...
    def calc_loss(
        self, x, y, y_hat, latent, target_ages, input_ages, no_aging, data_type="real"
    ):
        loss_dict = {}
        id_logs = []
        loss = 0.0
        if self.opts.id_lambda > 0:
            weights = None
            if (
                self.opts.use_weighted_id_loss
            ):  # compute weighted id loss only on forward pass
        ...
        if self.opts.aging_lambda > 0:
            aging_loss, id_logs = self.aging_loss(
                y_hat, y, target_ages, id_logs, label=data_type
            )
    ...
    def parse_and_log_images(
        self, id_logs, x, y, y_hat, y_recovered, title, subscript=None, display_count=2
    ):
        im_data = []
    ...
    def log_images(self, name, im_data, subscript=None, log_latest=False):
        ...
        if subscript:
            path = os.path.join(
                self.logger.log_dir, name, "{}_{:04d}.jpg".format(subscript, step)
            )
        else:
